...
class RutaService:
    ...
    def create_ruta(self, ruta_create: RutaCreate) -> Ruta:
        ...
        if not unidad.is_active:
            raise HTTPException(status_code=400, detail="Unidad is not active")
        ...
        if ruta_create.estimated_time_hours <= 0:
            raise HTTPException(status_code=400, detail="Estimated time must be greater than 0")
    ...
    def update_ruta(self, ruta_id: int, ruta_update: RutaUpdate) -> Optional[Ruta]:
        ...
        if ruta.status == RouteStatus.COMPLETADA:
            raise HTTPException(status_code=400, detail="Cannot update a completed route")
    ...
    def update_ruta_status(self, ruta_id: int, status_update: RutaStatusUpdate) -> Optional[Ruta]:
        ...
        if current_status == RouteStatus.COMPLETADA:
            raise HTTPException(status_code=400, detail="Cannot change status of a completed route")

        if current_status == RouteStatus.CANCELADA:
            raise HTTPException(status_code=400, detail="Cannot change status of a cancelled route")
    ...
    def delete_ruta(self, ruta_id: int) -> bool:
        ...
        if ruta.status == RouteStatus.EN_RUTA:
            raise HTTPException(status_code=400, detail="Cannot delete a route that is in progress")
